...
password = 'wahyuardiva2005'
cxn_str = f'mongodb+srv://wahyuardiva82:{password}@cluster0.evtawsc.mongodb.net/?retryWrites=true&w=majority'
client = MongoClient(cxn_str, tlsCAFile=certifi.where())
db = client.dbsparta_plus_UKK
...
def get_restaurants():
    restaurants = list(db.restoran.find({}, {'_id': False}))
...
def delete_restaurant():
    name = request.form.get('name')
    db.restoran.delete_one({'name': name})
    return jsonify({
        'result': 'success',
